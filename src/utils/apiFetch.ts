...
/**
 * Hàm apiFetch mở rộng từ Fetch API
 * Tự động xử lý lỗi và quản lý headers
 * @param endpoint Đường dẫn API endpoint
 * @param options Các tùy chọn cho fetch request
...
const apiFetch = async (endpoint: string, options: FetchOptions = {}) => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new FetchApiError(response.status, await response.json().catch(() => null));
    }
